...
crm_off <- read.table("on_vs_off2.tsv", header = TRUE) #training vs testing for on/off
test_test <- read.table("temp_test.tsv", header = TRUE) #putative enhancers (based on Dl binding) around brk, vnd, zen, sog
...
write.table(new_data_frame, "R_export1.txt", sep="\t")
