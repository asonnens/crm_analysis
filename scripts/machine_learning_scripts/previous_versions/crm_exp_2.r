...
svm_enhancers_linear <- function(dataset, datatable, variable_pos, variable_name){
	...
	svm_model <- svm(Ectoderm ~., data = svm_training, kernel = "linear")
...
crm_exp <- read.table("expression_100.tsv", header = TRUE) 
...
ectoderm_rf_500 <- randomForest(Ectoderm ~., data = ectoderm_training, ntree = 500)
ectoderm_svm_linear <- svm(Ectoderm ~., data = ectoderm_training, kernel = "linear")
ectoderm_svm_radial <- svm(Ectoderm ~., data = ectoderm_training, kernel = "radial")
ectoderm_svm_sigmoid <- svm(Ectoderm ~., data = ectoderm_training, kernel = "sigmoid")
endoderm_rf_500 <- randomForest(Endoderm ~., data = endoderm_training, ntree = 500)
endoderm_svm_linear <- svm(Endoderm ~., data = endoderm_training, kernel = "linear")
endoderm_svm_radial <- svm(Endoderm ~., data = endoderm_training, kernel = "radial")
endoderm_svm_sigmoid <- svm(Endoderm ~., data = endoderm_training, kernel = "sigmoid")
mesoderm_rf_500 <- randomForest(Mesoderm ~., data = mesoderm_training, ntree = 500)
mesoderm_svm_linear <- svm(Mesoderm ~., data = mesoderm_training, kernel = "linear")
mesoderm_svm_radial <- svm(Mesoderm ~., data = mesoderm_training, kernel = "radial")
mesoderm_svm_sigmoid <- svm(Mesoderm ~., data = mesoderm_training, kernel = "sigmoid")
