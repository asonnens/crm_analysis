...
#testing and training sets returned as a list
  ...
  temp_scale <- scale(dataset[,start_position:dataframe_length])
  my_data <- data.frame(cbind(dataset[,"status"], temp_scale))
  my_data_unscaled <- dataset[, c(variable_pos, start_position:dataframe_length)]
...
svm_enhancers_fun_linear <- function(dataset, datatable, kernel_type = "linear"){
    svm_enhancers <- strata_5var(dataset, "status", datatable, 2/3, 2, 10, 8, 150)
...
svm_enhancer_comparison_linear <- resample_default(svm_enhancers_fun_linear, crm_off, tab_on, 10)
