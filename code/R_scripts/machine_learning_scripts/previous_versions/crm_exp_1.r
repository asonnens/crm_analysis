...
resample_default <- function(function_name, dataset, datatable, reps, variable_pos = 2, variable_name = "status"){
	out_list <- list()
    for(i in 1:reps){
...
svm_enhancers_fun_linear <- function(dataset, datatable, variable_pos = 2, variable_name = "status"){
    svm_enhancers <- strata_5var(dataset, variable_name, datatable, 100, variable_pos, 10, 7, 150)
	svm_training <- svm_enhancers[[1]]
	svm_test <- svm_enhancers[[2]]
	svm_model <- svm(status ~., data = svm_training, kernel = "linear")
...
svm_enhancer_comparison_linear <- resample_default(svm_enhancers_fun_linear, crm_off, tab_on, 100, 2, "status")
svm_enhancer_comparison_linear #82%
svm_ectoderm_linear <- resample_default(svm_enhancers_fun_linear, crm_exp, tab_ectoderm, 50, 8, "Ectoderm")
svm_ectoderm_linear
...
rf_enhancer_comparison_100 #about 73%
rf_enhancer_comparison_500 <- resample_default(rf_enhancer_fun_500, crm_off, tab_on, 100)
rf_enhancer_comparison_500 #about 73%
rf_enhancer_comparison_1000 <- resample_default(rf_enhancer_fun_1000, crm_off, tab_on, 100)
rf_enhancer_comparison_1000 #about 74%

crm_on_svm <- svm(status ~ ., data = on_training, kernel = "linear")
...
crm_exp_train_test <- strata_5var(crm_exp, "Ectoderm", tab_on, 2/3, 8, 21, 10, 15)
