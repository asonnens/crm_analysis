...
Always_never = open("Always_never.tsv")
all_data = open("all_data1.tsv")
Always_never_out = "Always_never_NA.tsv"
all_data_out = "all_data.tsv"
...
def fix_always_never(always_never_file, outfile_name):
    #num_lines = sum(1 for line in always_never_file)
    always_never_file.seek(0)
    first_line = always_never_file.readline()
    ...
    for i in range(2,len(first_line_list)):
        ...
        print count
        temp_list = []
        for line in always_never_file:
            linelist = line.split("\t")
            temp_list.append(float(linelist[count]))
            if count == 2:
                ID_list.append(linelist[0])
                status_list.append(linelist[1])
        print np.min(temp_list), np.std(temp_list), str(np.min(temp_list) - np.std(temp_list))
        min_val = str(np.min(temp_list) - np.std(temp_list))
        out_i = [str(x) if x != 0 else str(min_val) for x in temp_list]
        if count == 2:
            big_list.append(ID_list)
            big_list.append(status_list)
        big_list.append(out_i)
        count = count + 1        
    outfile = open(outfile_name, "w")
    outfile.write(first_line)
    for i in range(0,len(big_list[1])):
        newlist = [x[i] for x in big_list]
        outstring = "\t".join(newlist)
        outfile.write(outstring)
        outfile.write("\r\n")
    always_never_file.close()
    outfile.close()
    

#fix_always_never(Always_never, Always_never_out)
#fix_always_never(all_data, all_data_out)
#fix_always_never(Yep_file, "Active_fixed")
#fix_always_never(Nope_file, "Inactive_fixed")
#Nope_file.close()
#Yep_file.close()
Nope_file = open("Inactive.tsv")
Yep_file = open("Active.tsv")
