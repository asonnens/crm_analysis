...
#random_balance <-cbind(all_data_balance[,0:2],random_balance)
...
resample_rf500 <- function(function_name, dataset, datatable, variable_name, outfilename, description="README!", reps, samplesize, stratafun, graph, validated_data, info, featurenum, format, model, balance){
    ...
    for(i in 1:reps){
		...
		temp_file_name <- paste(temp_file_name, ".csv", sep = "")
		temp_file_name <- paste(temp_dir, temp_file_name, sep = "/")
        myresult <- function_name(dataset, datatable, temp_file_name, info_file_name, samplesize, stratafun = strata_Percent_var, count, graph, mydataframe, validated_data, info, featurenum,format, model, balance)
...
enhancer_class_status_scale <- function(dataset, datatable, filename = "temp", summaryfile = "temp_summary", samplesize, stratafun, count, graph, mydataframe, validated_data, info, featurenum, format, model, balance){
    balance_data <- dataset
	balancetable <- table(balance_data$status)
    enhancers_set <- stratafun(balance_data, "status", balancetable,2, featurenum, featurenum - 3, samplesize)   
	enhancers_training2 <- enhancers_set[[1]]
...
current_all <- all_data
current_tab <- table(all_data$status)
all_PRROC_data_bal <- resample_rf500(enhancer_class_status_balance, current_all, current_tab, "allData_prosp", description = "allData_prosp", "allData_prosp", 10, 250, strata_Percent_var, "recall", prosp_target, "All_Data_Test_prosp",43, "NA", "NA", "T")
all_means_bal <- apply(all_PRROC_data_bal[[1]], MARGIN = 2, FUN = mean)
...
group_by(status) %>%
sample_n(size = 300)
all_data_balance <- all_data_balance %>% na.omit()
all_data_balance <- all_data_balance %>%  droplevels()
all_data__balance <- data.frame(all_data_balance)
...
all_bal_PRROC_data <- resample_rf500(enhancer_class_status_balance, current_all, current_tab, "allData_balance_prosp", description = "allData_balance_prosp", "allData_balance_prosp", 10, 250, strata_Percent_var, "recall", prosp_target, "allData_balance_Test_prosp",43, "NA", "NA")

all_bal_means <- apply(all_bal_PRROC_data[[1]], MARGIN = 2, FUN = mean)
...
Dl_chip_balance = Dl_chip_balance %>% na.omit()
group_by(status) %>%
sample_n(size = 300)
...
current_all <- Dl_chip
#current_tab <- table(current_all$status)
Dlchip_bal_ROC_data <- resample_rf500(enhancer_class_status_balance, current_all, current_tab, "Dl_chip_balance", description = "Dl_chip_balance", "Dl_chip_balance", 10, 250, strata_Percent_var, "ROC", current_target, "Dl_chip_balance_Test",43, "NA", "NA", "T")
...
current_all <- Dl_Twi
current_tab <- table(current_all$status)
DlTwi_bal_ROC_data <- resample_rf500(enhancer_class_status_balance, current_all, current_tab, "Dl_Twi_chip_balance", description = "Dl_Twi_chip_balance", "Dl_Twi_chip_balance", 10, 250, strata_Percent_var, "ROC", current_target, "Dl_Twi_chip_balance",43, "NA", "NA", "T")
DlTwi_bal_means <- apply(DlTwi_bal_ROC_data[[1]], MARGIN = 2, FUN = mean)
DlTwi_bal_sd <- apply(DlTwi_bal_ROC_data[[1]], MARGIN=2, FUN=sd)
DlTwi_bal_PRROC_data <- resample_rf500(enhancer_class_status_scale, current_all, current_tab, "Dl_Twi_balance_prosp", description = "Dl_Twi_balance_prosp", "Dl_Twi_balance_prosp", 10, 250, strata_Percent_var, "recall", prosp_target, "Dl_Twi_balance_Test_prosp",43, "NA", "NA", "T")
...
current_tab <- table(current_all$status)
Dl15_all_ROC_data <- resample_rf500(enhancer_class_status_scale, current_all, current_tab, "Dl15_train_all", description = "Dl15_train_all", "Dl15_train_all", 10, 250, strata_Percent_var, "recall", current_target, "Dl15_train_all_Test",43, "NA", "NA")
...
current_all <- Dl15_train_all
current_tab <- table(current_all$status)
Dl15_bal_ROC_data <- resample_rf500(enhancer_class_status_balance, current_all, current_tab, "Dl15_train_all", description = "Dl15_train_all", "Dl15_train_all", 10, 100, strata_Percent_var, "recall", current_target, "Dl15_train_all_Test",43, "NA", "NA", "T")
Dl15_bal_means <- apply(Dl15_bal_ROC_data[[1]], MARGIN = 2, FUN = mean)
Dl15_bal_sd <- apply(Dl15_bal_ROC_data[[1]], MARGIN=2, FUN=sd)
...
current_all <- H3K4me1_train_all
current_tab <- table(current_all$status)
H3K4me1_ROC_data_bal <- resample_rf500(enhancer_class_status_balance, current_all, current_tab, "H3K4me1_train_all", description = "H3K4me1_train_all", "H3K4me1_train_all", 10, 150, strata_Percent_var, "ROC", current_target, "H3K4me1_train_all_Test",43, "NA", "NA", "T")
H3K4me1_all_means_bal <- apply(H3K4me1_ROC_data_bal[[1]], MARGIN = 2, FUN = mean)
...
current_all <- FTwi_train_all
current_tab <- table(current_all$status)
Twi14_ROC_data_bal <- resample_rf500(enhancer_class_status_balance, current_all, current_tab, "FTwi14_train_all", description = "FTwi14_train_all", "FTwi14_train_all", 10, 250, strata_Percent_var, "ROC", current_target, "FTwi14_train_all_Test",43, "NA", "NA", "T")
Twi14_all_means_bal <- apply(Twi14_ROC_data_bal[[1]], MARGIN = 2, FUN = mean)
...
current_all <- Zld_train_all
current_tab <- table(current_all$status)
Zld_bal_ROC_data <- resample_rf500(enhancer_class_status_balance, current_all, current_tab, "Zld_train_all", description = "Zld_train_all", "Zld_train_all", 10, 250, strata_Percent_var, "ROC", current_target, "Zld_train_all_Test",43, "NA", "NA")
Zld_bal_means <- apply(Zld_bal_ROC_data[[1]], MARGIN = 2, FUN = mean)
...
current_all <- Bcd13_train_all
current_tab <- table(current_all$status)
Bcd13_bal_ROC_data <- resample_rf500(enhancer_class_status_balance, current_all, current_tab, "Bcd13_train_all", description = "Bcd13_train_all", "Bcd13_train_all", 10, 100, strata_Percent_var, "ROC", current_target, "Bcd13_train_all_Test",43, "NA", "NA")
Bcd13_bal_means <- apply(Bcd13_bal_ROC_data[[1]], MARGIN = 2, FUN = mean)


current_all <- Bcd13_train_cons
current_tab <- table(current_all$status)
Bcd13_cons_ROC_data <- resample_rf500(enhancer_class_status_scale, current_all, current_tab, "Bcd13_train_cons", description = "Bcd13_train_cons", "Bcd13_train_cons", 10, 100, strata_Percent_var, "ROC", current_target, "Bcd13_train_cons_Test",43, "NA", "NA")
...
current_all <- Bcd09_train_all
current_tab <- table(current_all$status)
Bcd09_bal_ROC_data <- resample_rf500(enhancer_class_status_balance, current_all, current_tab, "Bcd09_train_all", description = "Bcd09_train_all", "Bcd09_train_all", 10, 100, strata_Percent_var, "ROC", current_target, "Bcd09_train_all_Test",43, "NA", "NA")
Bcd09_bal_means <- apply(Bcd09_bal_ROC_data[[1]], MARGIN = 2, FUN = mean)
...
current_all <- Cad09_train_all
current_tab <- table(current_all$status)
Cad09_bal_ROC_data <- resample_rf500(enhancer_class_status_balance, current_all, current_tab, "Cad09_train_all", description = "Cad09_train_all", "Cad09_train_all", 10, 100, strata_Percent_var, "ROC", current_target, "Cad09_train_all_Test",43, "NA", "NA")
Cad09_bal_means <- apply(Cad09_bal_ROC_data[[1]], MARGIN = 2, FUN = mean)
...
current_all <- Gt13_train_all
current_tab <- table(current_all$status)
Gt13_bal_ROC_data <- resample_rf500(enhancer_class_status_balance, current_all, current_tab, "Gt13_train_all", description = "Gt13_train_all", "Gt13_train_all", 10, 100, strata_Percent_var, "ROC", current_target, "Gt13_train_all_Test",43, "NA", "NA")
Gt13_bal_means <- apply(Gt13_bal_ROC_data[[1]], MARGIN = 2, FUN = mean)
...
current_all <- Gt09_train_all
current_tab <- table(current_all$status)
Gt09_bal_ROC_data <- resample_rf500(enhancer_class_status_balance, current_all, current_tab, "Gt09_train_all", description = "Gt09_train_all", "Gt09_train_all", 10, 100, strata_Percent_var, "ROC", current_target, "Gt09_train_all_Test",43, "NA", "NA")
Gt09_bal_means <- apply(Gt09_bal_ROC_data[[1]], MARGIN = 2, FUN = mean)
...
current_all <- Kr13_train_all
current_tab <- table(current_all$status)
Kr13_bal_ROC_data <- resample_rf500(enhancer_class_status_balance, current_all, current_tab, "Kr13_train_all", description = "Kr13_train_all", "Kr13_train_all", 10, 100, strata_Percent_var, "ROC", current_target, "Kr13_train_all_Test",43, "NA", "NA")
Kr13_bal_means <- apply(Kr13_bal_ROC_data[[1]], MARGIN = 2, FUN = mean)
...
current_all <- Hry09_train_all
current_tab <- table(current_all$status)
Hry09_bal_ROC_data <- resample_rf500(enhancer_class_status_balance, current_all, current_tab, "Hry09_train_all", description = "Hry09_train_all", "Hry09_train_all", 10, 100, strata_Percent_var, "ROC", current_target, "Hry09_train_all_Test",43, "NA", "NA")
Hry09_bal_means <- apply(Hry09_bal_ROC_data[[1]], MARGIN = 2, FUN = mean)
...
Random_means <- apply(Random_PRROC_data[[1]], MARGIN = 2, FUN = mean)
...
Random_bal_means <- apply(Random_bal_PRROC_data[[1]], MARGIN = 2, FUN = mean)
...
all_models_bal <- c(Twi14_all_means[1]/100,Twi14_all_means[6]   + 1, Twi14_all_means[7]   + 2 ,
          Twi14_all_means_bal[1]/100,Twi14_all_means_bal[6]   + 1, Twi14_all_means_bal[7]   + 2 ,Twi14_cons_means[1]/100,Twi14_cons_means[6]   + 1, Twi14_cons_means[7]   + 2 ,
		  Zld_all_means[1]/100, Zld_all_means[6] +1, Zld_all_means[7]   + 2,
		  Zld_bal_means[1]/100, Zld_bal_means[6] +1, Zld_bal_means[7]   + 2, Zld_cons_means[1]/100, Zld_cons_means[6] +1, Zld_cons_means[7]   + 2,
		  H3K4me1_all_means[1]/100, H3K4me1_all_means[6] + 1 , H3K4me1_all_means[7] +2,
		  H3K4me1_all_means_bal[1]/100, H3K4me1_all_means_bal[6] + 1 , H3K4me1_all_means_bal[7] +2 , H3K4me1_cons_means[1]/100, H3K4me1_cons_means[6] + 1 , H3K4me1_cons_means[7] +2 , 
		  Bcd13_all_means[1]/100,Bcd13_all_means[6]   + 1,Bcd13_all_means[7]   + 2,
		  Bcd13_bal_means[1]/100,Bcd13_bal_means[6]   + 1,Bcd13_bal_means[7]   + 2, Bcd13_cons_means[1]/100,Bcd13_cons_means[6]   + 1,Bcd13_cons_means[7]   + 2
		  )
...
xnames <- c(1,1,1,3,3,3,7,7,7,9,9,9,13,13,13,15,15,15,19,19,19,21,21,21,25,25,25,27,27,27)
xnames2 <- c(1,1,1,3,3,3,5,5,5,9,9,9,11,11,11,13,13,13,17,17,17,19,19,19,21,21,21,25,25,25,27,27,27,29,29,29)
plot(xnames, all_models, xaxt = "none", pch = 19, col = c("black","black", "black","cyan4","cyan4","cyan4"), ylim = c(0.1, 2.9),xlim = c(1,30),yaxt = "none", xlab = "")
...
plot(xnames2, all_models_bal, xaxt = "none", pch = 19, col = c("black", "black", "black","darkred","darkred", "darkred","cyan4","cyan4","cyan4"), ylim = c(0.1, 2.9),xlim = c(1,30),yaxt = "none", xlab = "")
abline(h=1,col=1,lty=1)
abline(h=2,col=1,lty=1)
abline(h=0.5,col="gray",lty=2)
abline(h=1.5,col="gray",lty=2)
...
lab = c("Twi_14_c", "Zld_11_s","H3K4me1_15", "Bcd_13_s")

text(x=c(2,8,14,20,26), y=par()$usr[3] - 0.01*(par()$usr[4]-par()$usr[3]),labels = lab,srt=45, adj=1, xpd=TRUE)
lab = c("Twi_14_c", "Zld_11_s","H3K4me1_15", "Bcd_13_s")
text(x=c(3,11,19,27), y=par()$usr[3] - 0.01*(par()$usr[4]-par()$usr[3]),labels = lab,srt=45, adj=1, xpd=TRUE)
all_models_base <- c(
          all_means[1]/100,all_means[8]   + 1,all_means[9]   + 2, all_bal_means[1]/100,all_bal_means[8]   + 1,all_bal_means[9]   + 2, 
          Twi14_all_means[1]/100,Twi14_all_means[8]   + 1, Twi14_all_means[9]   + 2 ,Twi14_cons_means[1]/100,Twi14_cons_means[8]   + 1, Twi14_cons_means[9]   + 2 ,
		  Zld_all_means[1]/100, Zld_all_means[8] +1, Zld_all_means[9]   + 2, Zld_cons_means[1]/100, Zld_cons_means[8] +1, Zld_cons_means[9]   + 2,
		  H3K4me1_all_means[1]/100, H3K4me1_all_means[8] + 1 , H3K4me1_all_means[9] +2 , H3K4me1_cons_means[1]/100, H3K4me1_cons_means[8] + 1 , H3K4me1_cons_means[9] +2 , 
		  Bcd13_all_means[1]/100,Bcd13_all_means[8]   + 1,Bcd13_all_means[9]   + 2, Bcd13_cons_means[1]/100,Bcd13_cons_means[8]   + 1,Bcd13_cons_means[9]   + 2
		  )

plot(xnames, all_models_base, xaxt = "none", pch = 19, col = c("black","black", "black","cyan4","cyan4","cyan4"), ylim = c(0.1, 2.9),xlim = c(1,30),yaxt = "none", xlab = "")
abline(h=1,col=1,lty=1)
abline(h=2,col=1,lty=1)
abline(h=0.5,col="gray",lty=2)
abline(h=1.5,col="gray",lty=2)
...
lab = c("all_data","Twi_14_c", "Zld_11_s","H3K4me1_15", "Bcd_13_s")
...
all_models_bal_base <- c(Twi14_all_means[1]/100,Twi14_all_means[8]   + 1, Twi14_all_means[9]   + 2 ,
          Twi14_all_means_bal[1]/100,Twi14_all_means_bal[8]   + 1, Twi14_all_means_bal[9]   + 2 ,Twi14_cons_means[1]/100,Twi14_cons_means[8]   + 1, Twi14_cons_means[9]   + 2 ,
		  Zld_all_means[1]/100, Zld_all_means[8] +1, Zld_all_means[9]   + 2,
		  Zld_bal_means[1]/100, Zld_bal_means[8] +1, Zld_bal_means[9]   + 2, Zld_cons_means[1]/100, Zld_cons_means[8] +1, Zld_cons_means[9]   + 2,
		  H3K4me1_all_means[1]/100, H3K4me1_all_means[8] + 1 , H3K4me1_all_means[9] +2,
		  H3K4me1_all_means_bal[1]/100, H3K4me1_all_means_bal[8] + 1 , H3K4me1_all_means_bal[9] +2 , H3K4me1_cons_means[1]/100, H3K4me1_cons_means[8] + 1 , H3K4me1_cons_means[9] +2 , 
		  Bcd13_all_means[1]/100,Bcd13_all_means[8]   + 1,Bcd13_all_means[9]   + 2,
		  Bcd13_bal_means[1]/100,Bcd13_bal_means[8]   + 1,Bcd13_bal_means[9]   + 2, Bcd13_cons_means[1]/100,Bcd13_cons_means[8]   + 1,Bcd13_cons_means[9]   + 2
		  )
plot(xnames2, all_models_bal_base, xaxt = "none", pch = 19, col = c("black", "black", "black","darkred","darkred", "darkred","cyan4","cyan4","cyan4"), ylim = c(0.1, 2.9),xlim = c(1,30),yaxt = "none", xlab = "")
abline(h=1,col=1,lty=1)
abline(h=2,col=1,lty=1)
abline(h=0.5,col="gray",lty=2)
abline(h=1.5,col="gray",lty=2)
...
supplementary_models <- c( 
          all_means[1]/100,all_means[8]   + 1,all_means[9]   + 2, all_bal_means[1]/100,all_bal_means[8]   + 1,all_bal_means[9]   + 2, 
		  Dl15_all_means[1]/100, Dl15_all_means[8]   + 1,Dl15_all_means[9]   + 2 , Dl15_cons_means[1]/100, Dl15_cons_means[8]   + 1,Dl15_cons_means[9]   + 2,   
		  Bcd09_all_means[1]/100, Bcd09_all_means[8]   + 1, Bcd09_all_means[9]   + 2, Bcd09_cons_means[1]/100, Bcd09_cons_means[8]   + 1, Bcd09_cons_means[9]   + 2, 
          Cad09_all_means[1]/100,Cad09_all_means[8]   + 1, Cad09_all_means[9]   + 2 ,Cad09_cons_means[1]/100,Cad09_cons_means[8]   + 1, Cad09_cons_means[9]   + 2 ,
		  Gt13_all_means[1]/100, Gt13_all_means[8] +1, Gt13_all_means[9]   + 2, Gt13_cons_means[1]/100, Gt13_cons_means[8] +1, Gt13_cons_means[9]   + 2,
          Gt09_all_means[1]/100, Gt09_all_means[8]   + 1, Gt09_all_means[9]   + 2 , Gt09_cons_means[1]/100, Gt09_cons_means[8]   + 1, Gt09_cons_means[9]   + 2,
		  Kr13_all_means[1]/100, Kr13_all_means[8]   + 1, Kr13_all_means[9]   + 2 , Kr13_cons_means[1]/100, Kr13_cons_means[8]   + 1, Kr13_cons_means[9]   + 2,
		  Hry09_all_means[1]/100, Hry09_all_means[8]   + 1,Hry09_all_means[9]   + 2 , Hry09_cons_means[1]/100, Hry09_cons_means[8]   + 1,Hry09_cons_means[9]   + 2 )
		  
xnames3 <- c(1,1,1,3,3,3,7,7,7,9,9,9,13,13,13,15,15,15,19,19,19,21,21,21,25,25,25,27,27,27,31,31,31,33,33,33,37,37,37,39,39,39,43,43,43,45,45,45)
plot(xnames3, supplementary_models, xaxt = "none", pch = 19, col = c("black","black", "black","cyan4","cyan4","cyan4"), ylim = c(0.1, 2.9),xlim = c(1,46),yaxt = "none", xlab = "")
abline(h=1,col=1,lty=1)
abline(h=2,col=1,lty=1)
abline(h=0.5,col="gray",lty=2)
abline(h=1.5,col="gray",lty=2)
...
lab = c("all_data", "D1_15_s", "Bcd_09_c", "Cad_09_c", "Gt_13_s", "Gt_09_c", "Kr_13_s", "Hry_09_c")

text(x=c(2,8,14,20,26,32,38,44), y=par()$usr[3] - 0.01*(par()$usr[4]-par()$usr[3]),labels = lab,srt=45, adj=1, xpd=TRUE)

		  
supplementary_models_base_bal <- c( 
		  Dl15_bal_means[1]/100, Dl15_bal_means[8]   + 1,Dl15_bal_means[9]   + 2, Dl15_cons_means[1]/100, Dl15_cons_means[8]   + 1,Dl15_cons_means[9]   + 2,   
		  Bcd09_bal_means[1]/100, Bcd09_bal_means[8]   + 1, Bcd09_bal_means[9]   + 2,Bcd09_cons_means[1]/100, Bcd09_cons_means[8]   + 1, Bcd09_cons_means[9]   + 2, 
          Cad09_bal_means[1]/100,Cad09_bal_means[8]   + 1, Cad09_bal_means[9]   + 2 ,Cad09_cons_means[1]/100,Cad09_cons_means[8]   + 1, Cad09_cons_means[9]   + 2 ,
		  Gt13_bal_means[1]/100, Gt13_bal_means[8] +1, Gt13_bal_means[9]   + 2, Gt13_cons_means[1]/100, Gt13_cons_means[8] +1, Gt13_cons_means[9]   + 2,
          Gt09_bal_means[1]/100, Gt09_bal_means[8]   + 1, Gt09_bal_means[9]   + 2 , Gt09_cons_means[1]/100, Gt09_cons_means[8]   + 1, Gt09_cons_means[9]   + 2,
		  Kr13_bal_means[1]/100, Kr13_bal_means[8]   + 1, Kr13_bal_means[9]   + 2 , Kr13_cons_means[1]/100, Kr13_cons_means[8]   + 1, Kr13_cons_means[9]   + 2,
		  Hry09_bal_means[1]/100, Hry09_bal_means[8]   + 1,Hry09_bal_means[9]   + 2, Hry09_cons_means[1]/100, Hry09_cons_means[8]   + 1,Hry09_cons_means[9]   + 2 )
		  
xnames4 <- c(1,1,1,3,3,3,7,7,7,9,9,9,13,13,13,15,15,15,19,19,19,21,21,21,25,25,25,27,27,27,31,31,31,33,33,33,37,37,37,39,39,39)
plot(xnames4, supplementary_models_base_bal, xaxt = "none", pch = 19, col = c("darkred","darkred", "darkred","cyan4","cyan4","cyan4"), ylim = c(0.1, 2.9),xlim = c(1,40),yaxt = "none", xlab = "")
abline(h=1,col=1,lty=1)
abline(h=2,col=1,lty=1)
abline(h=0.5,col="gray",lty=2)
abline(h=1.5,col="gray",lty=2)
...
lab = c("D1_15_s", "Bcd_09_c", "Cad_09_c", "Gt_13_s", "Gt_09_c", "Kr_13_s", "Hry_09_c")

text(x=c(2,8,14,20,26,32,38), y=par()$usr[3] - 0.01*(par()$usr[4]-par()$usr[3]),labels = lab,srt=45, adj=1, xpd=TRUE)
