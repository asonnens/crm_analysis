...
def peak_overlap(smaller_file, bigger_file):
    ...
    for line in smaller_file:
        ...
        file1_chr = file1_info[1]
        file1_coord1 = int(file1_info[2])
        file1_coord2 = int(file1_info[3])
        for line2 in bigger_file:
            file2_info = line2.split("\t")
            if file2_info[1] != file1_chr:
                pass
            else:
                if (((int(file2_info[2]) > file1_coord2)) or (file1_coord1 > int(file2_info[3]))):
                    pass
                else:
...
def file_stats(filename1, filename2, fileinfo = ""):
    ...
    if file2_length > file1_length:
        difference_filesize = 100 * (file2_length - file1_length)/file2_length
        percent_overlap = 100 * peak_overlap(file1, file2)/file1_length
    else:
        difference_filesize = 100 *  (file1_length - file2_length)/file1_length
...
def each_file_stats(filename, fileinfo = ""):
    ...
    for line in file1:
        info = line.split("\t")
        bound_region_list.append(float(info[4]))
...
#file_stats("twi_1_081506-sym-1_table.txt", "twi_2_081506-sym-1_table.txt", "twist files")
...
#each_file_stats("twi_1_081506-sym-1_table.txt", "twist file 1")
#each_file_stats("twi_2_081506-sym-1_table.txt", "twist file 2")
...
each_file_stats("twi_1_081506-sym-25_table.txt", "twist file 25 1")
each_file_stats("twi_2_081506-sym-25_table.txt", "twist file 25 2")
